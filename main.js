...
function runCakeSequence() {
  ...
  gsap.set([".layer-bottom", ".layer-middle", ".layer-top"], {
    y: -800,
    opacity: 1,
    scaleY: 1.5,
  });
  ...
  tl.to(".layer-bottom", { y: 0, scaleY: 1, duration: 0.6, ease: "bounce.out" })
    .to(
      ".layer-middle",
      { y: 0, scaleY: 1, duration: 0.5, ease: "bounce.out" },
      "-=0.35",
    )
    .to(
      ".layer-top",
      { y: 0, scaleY: 1, duration: 0.4, ease: "bounce.out" },
      "-=0.3",
    )
    .to(".layer", {
      scaleY: 0.9,
...
// --- 4. คลิกเค้ก (จุดรวมการแก้บัค Text) ---
cakeContainer.addEventListener("click", () => {
  // เคลียร์อนิเมชั่นที่ค้างอยู่ทั้งหมดของเปลวไฟและข้อความ
  ...
  if (isCandleLit) {
    ...
    // แก้บัค: ข้อความคำแนะนำหายไปทันทีและไม่ทับซ้อน
    ...
    // แก้บัค: แสดงข้อความคำแนะนำกลับมาอย่างถูกต้อง
    gsap.to(instruction, {
      opacity: 1,
      y: -10,
      duration: 0.4,
